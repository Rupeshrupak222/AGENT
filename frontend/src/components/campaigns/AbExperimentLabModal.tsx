  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 dark:bg-black/80 backdrop-blur-md animate-in fade-in">
      <div className="relative w-full max-w-4xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        {/* Header */}
        <div className="px-6 py-5 border-b border-slate-200 dark:border-white/10 flex items-center justify-between bg-purple-50/50 dark:bg-gradient-to-r dark:from-purple-900/30 dark:via-slate-900 dark:to-brand-950/30">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-purple-500/20 border border-purple-500/40 text-purple-600 dark:text-purple-400 flex items-center justify-center shadow-lg shadow-purple-500/20">
              <FlaskConical className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-black text-slate-900 dark:text-white tracking-tight">A/B Voice & Script Experimentation Lab</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="p-4 rounded-2xl bg-emerald-50 dark:bg-gradient-to-r dark:from-emerald-500/20 dark:via-brand-500/10 dark:to-transparent border border-emerald-200 dark:border-emerald-500/40 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-bold text-emerald-950 dark:text-white">Statistical Significance Achieved: 98.2% Confidence</h3>
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-mono font-bold bg-emerald-500/30 text-emerald-700 dark:text-emerald-300">
                    p = 0.018
                  </span>
                </div>
                <p className="text-xs text-emerald-900/80 dark:text-white/70 mt-0.5">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            {/* Variant A (Control) */}
            <div className="p-5 rounded-2xl bg-white dark:bg-black/30 border border-slate-200 dark:border-white/10 shadow-sm space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-600 dark:text-white/70 uppercase tracking-wider">Variant A (Control)</span>
                <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-100 dark:bg-white/10 text-slate-700 dark:text-white/60">
                  {trafficSplit}% Traffic
                </span>
              </div>

              <div>
                <h4 className="text-sm font-bold text-slate-900 dark:text-white">Elena — Consultative Concierge</h4>
                <p className="text-xs text-slate-500 dark:text-white/40 mt-0.5">Gentle discovery inquiry opening pitch.</p>
              </div>

              <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-white/5 text-xs text-slate-700 dark:text-slate-300 italic leading-relaxed">
              ...
              <div className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-200 dark:border-white/10 text-center">
                <div>
                  <p className="text-[10px] text-slate-500 dark:text-white/40">Pickup Rate</p>
                  <p className="text-xs font-mono font-bold text-slate-900 dark:text-white mt-0.5">46.2%</p>
                </div>
                <div>
                  <p className="text-[10px] text-slate-500 dark:text-white/40">Avg Duration</p>
                  <p className="text-xs font-mono font-bold text-slate-900 dark:text-white mt-0.5">2m 14s</p>
                </div>
                <div>
                  <p className="text-[10px] text-slate-500 dark:text-white/40">Booking Rate</p>
                  <p className="text-xs font-mono font-bold text-slate-900 dark:text-white mt-0.5">12.4%</p>
                </div>
              </div>
            </div>

            {/* Variant B (Challenger / Winner) */}
            <div className="p-5 rounded-2xl bg-purple-500/5 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-500/40 space-y-4 shadow-sm">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-purple-700 dark:text-purple-300 uppercase tracking-wider flex items-center gap-1.5">
                  <Trophy className="w-3.5 h-3.5 text-amber-500 dark:text-amber-400" /> Variant B (Champion Winner)
                </span>
              ...
              <div>
                <h4 className="text-sm font-bold text-slate-900 dark:text-white">Maya — High-Urgency Value Proposition</h4>
                <p className="text-xs text-purple-800/70 dark:text-purple-300/60 mt-0.5">Benchmark statistics hook with immediate ROI framing.</p>
              </div>

              <div className="p-3 rounded-xl bg-purple-500/10 dark:bg-purple-900/30 border border-purple-200 dark:border-purple-500/20 text-xs text-purple-900 dark:text-purple-100 italic leading-relaxed">
              ...
              <div className="grid grid-cols-3 gap-2 pt-2 border-t border-purple-200 dark:border-purple-500/20 text-center">
                <div>
                  <p className="text-[10px] text-purple-800/70 dark:text-purple-300/60">Pickup Rate</p>
                  <p className="text-xs font-mono font-bold text-emerald-600 dark:text-emerald-400 mt-0.5">54.8% (+18%)</p>
                </div>
                <div>
                  <p className="text-[10px] text-purple-800/70 dark:text-purple-300/60">Avg Duration</p>
                  <p className="text-xs font-mono font-bold text-emerald-600 dark:text-emerald-400 mt-0.5">3m 48s (+70%)</p>
                </div>
                <div>
                  <p className="text-[10px] text-purple-800/70 dark:text-purple-300/60">Booking Rate</p>
                  <p className="text-xs font-mono font-bold text-emerald-600 dark:text-emerald-400 mt-0.5">22.8% (+83%)</p>
                </div>
              </div>
            </div>
          </div>
