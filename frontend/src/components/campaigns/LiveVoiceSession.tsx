...
export function LiveVoiceSession({
  ...
  return (
    <div className="flex flex-col h-full space-y-4">
      ...
      <div className="flex items-center justify-between p-3.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-emerald-500/30 text-slate-900 dark:text-white shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {isBargedIn ? (
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-rose-500/20 text-rose-600 dark:text-rose-400 font-bold border border-rose-500/40 flex items-center gap-1 animate-pulse">
                  <ShieldAlert className="w-3 h-3" /> Live Takeover Active
                </span>
              ) : supervisorMode === "whisper" ? (
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 font-bold border border-indigo-500/40 flex items-center gap-1">
                  <Volume1 className="w-3 h-3" /> Whisper Coaching
                </span>
              ) : supervisorMode === "listen" ? (
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 font-bold border border-emerald-500/40 flex items-center gap-1">
                  <Headphones className="w-3 h-3" /> Silent Monitoring
                </span>
              ) : (
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 font-semibold border border-emerald-500/30 flex items-center gap-1">
                  <Radio className="w-3 h-3 animate-pulse" /> Live Telephony Sandbox
                </span>
              )}
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Customer: <span className="text-slate-800 dark:text-slate-200 font-medium">{customerName}</span>{" "}
        ...
        <div className="flex items-center gap-4">
          <div className="text-right font-mono">
            ...
            <p className="text-[10px] text-slate-500 dark:text-slate-400">Duration</p>
          </div>
          <button
      ...
      {supervisorNotice && (
        <div className="flex items-center gap-2 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-500/15 border border-emerald-200 dark:border-emerald-500/30 text-emerald-800 dark:text-emerald-300 text-xs">
          <span>{supervisorNotice}</span>
        </div>
      )}

      {/* Supervisor Live Coaching & Monitoring Bar */}
      <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 space-y-2.5">
          ...
          <button
            ...
            className={`flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${
              supervisorMode === "barge_in" || isBargedIn
                ...
                : "bg-slate-200/80 text-rose-700 hover:text-rose-800 hover:bg-rose-100 dark:bg-slate-800/60 dark:text-rose-400 dark:hover:text-rose-300 dark:hover:bg-rose-950/30"
            }`}
          >
            ...
            <div className="flex items-center gap-2">
              <div
                className={`w-2 h-2 rounded-full ${
                  isMicActive ? "bg-brand-500 animate-ping" : "bg-slate-400 dark:bg-slate-500"
                }`}
              />
              <span className="text-xs font-semibold text-slate-800 dark:text-slate-200">Your Voice (Mic)</span>
            </div>
            <span className="text-[11px] font-mono text-brand-600 dark:text-brand-300">
              {isMicActive ? `${micVolume}%` : "Inactive"}
            </span>
          </div>
          {/* Animated equalizer bars */}
          <div className="flex items-center justify-center gap-1.5 h-10 px-2 bg-slate-200/60 dark:bg-black/40 rounded-lg overflow-hidden">
              ...
              return (
                <div
                  ...
                  className={`w-1 rounded-full transition-all duration-75 ${
                    isMicActive ? "bg-gradient-to-t from-brand-500 to-indigo-400" : "bg-slate-400 dark:bg-slate-700"
                  }`}
                />
              );
            })}
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <div
                className={`w-2 h-2 rounded-full ${
                  isAISpeaking ? "bg-emerald-500 animate-ping" : "bg-slate-400 dark:bg-slate-500"
                }`}
              />
              <span className="text-xs font-semibold text-slate-800 dark:text-slate-200">{agentName} (Speaker)</span>
            </div>
            ...
            {Array.from({ length: 16 }).map((_, i) => {
              ...
              return (
                <div
                  ...
                  style={{ height: `${heightMultiplier}%` }}
                  className={`w-1 rounded-full transition-all duration-75 ${
                    isAISpeaking ? "bg-gradient-to-t from-emerald-500 to-teal-400" : "bg-slate-400 dark:bg-slate-700"
                  }`}
                />
              );
            })}
      ...
      <div className="flex-1 flex flex-col rounded-xl bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800/80 p-3 min-h-[260px] max-h-[340px]">
        <div className="flex items-center justify-between pb-2 mb-2 border-b border-slate-200 dark:border-slate-800/60 text-xs text-slate-600 dark:text-slate-400 font-semibold">
          <span className="flex items-center gap-1.5">
            <Activity className="w-3.5 h-3.5 text-brand-600 dark:text-brand-400" /> Real-Time Telephony Transcripts
          </span>
        ...
        <div
          ref={chatScrollRef}
          className="flex-1 overflow-y-auto space-y-3 pr-1 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-800"
        >
          {transcripts.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-slate-500 text-xs text-center p-6">
              <Zap className="w-6 h-6 mb-2 text-brand-500 opacity-60" />
              <p className="font-semibold text-slate-700 dark:text-slate-300">Voice pipeline initialized & ready</p>
            ...
            transcripts.map((t) => (
              ...
              >
                {t.speaker === "agent" && (
                  <div className="w-7 h-7 rounded-lg bg-emerald-600/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 flex items-center justify-center flex-shrink-0 text-xs font-bold">
                ...
                {t.speaker === "user" && (
                  <div className="w-7 h-7 rounded-lg bg-brand-600/20 text-brand-600 dark:text-brand-400 border border-brand-500/30 flex items-center justify-center flex-shrink-0 text-xs font-bold">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            {!isMicActive ? (
              ...
              <>
                <button
                  ...
                  className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-slate-200 hover:bg-slate-300 text-slate-800 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-200 font-semibold text-xs border border-slate-300 dark:border-slate-700 transition-all"
                >
                  <MicOff className="w-4 h-4 text-rose-500 dark:text-rose-400" /> Stop Mic
                </button>
                <button
            ...
            {isAISpeaking && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-amber-500/10 border border-amber-500/40 hover:bg-amber-500/20 text-amber-700 dark:text-amber-300 font-semibold text-xs transition-all"
              >
          ...
          <div className="text-[11px] text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
            <Volume2 className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> WebRTC Audio Stream Active
          </div>
        </div>
        ...
        <form onSubmit={handleSendManualText} className="flex gap-2">
          <input
            ...
            className="flex-1 px-3.5 py-2 text-xs rounded-xl bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-brand-500 transition-colors"
          />
          <button
            type="submit"
            disabled={!manualText.trim()}
            className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-brand-700 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-brand-400 disabled:opacity-40 disabled:cursor-not-allowed font-semibold text-xs flex items-center gap-1.5 border border-slate-200 dark:border-slate-700 transition-colors"
          >
