...
Period Scope: ${report.range.toUpperCase()}
...
- Top Performing Voice Persona: ${topPerformingAgent ? `${topPerformingAgent.name} (${topPerformingAgent.completedCalls} calls)` : "Adyapan AI"}
...
-----------------------------------------------------
${report.funnel.map(f => `- ${f.stage.toUpperCase()}: ${f.count.toLocaleString()} leads (${f.pct}%)`).join("\n")}
...
=====================================================`;
      ...
      link.href = url;
      link.download = `AgentCall_AI_Executive_Report_${report.range}_${Date.now()}.txt`;
      link.click();
