...
import {
  ...
  TrendingUp, DollarSign, Clock, Download, Calculator, CheckCircle2, ArrowUpRight,
} from "lucide-react";
import {
...
export default function AnalyticsPage() {
  ...
  const humanCost = Math.round(targetCallsPerMonth * avgCallMinutes * 0.40);
  const aiCost = Math.round(targetCallsPerMonth * avgCallMinutes * 0.11);
  const netSavings = humanCost - aiCost;
  const savingsPct = Math.round((netSavings / Math.max(1, humanCost)) * 100);

  const exportExecutiveReport = () => {
    const totalCalls = metrics?.totalCalls || 0;
    const completedCalls = metrics?.connected || 0;
    const avgDuration = metrics?.avgDuration || 60;
    const avgSentiment = metrics?.avgSentiment || 0.88;

    const reportContent = `=====================================================
AGENTCALL AI — EXECUTIVE ROI & REVENUE ANALYTICS REPORT
=====================================================
Generated At: ${new Date().toLocaleString()}
Period Scope: ${period.toUpperCase()}

1. EXECUTIVE KPI SUMMARY
-----------------------------------------------------
- Total Outbound & Inbound Calls: ${totalCalls.toLocaleString()}
- Connected & Completed Conversations: ${completedCalls.toLocaleString()}
- Average Call Duration: ${avgDuration} seconds
- Customer Sentiment Index: ${(avgSentiment * 100).toFixed(1)}% Positive
- Top Performing Voice Persona: ${topAgent ? `${topAgent.name} (${topAgent.completedCalls} calls)` : "Adyapan AI"}

2. BPO REPLACEMENT & COST SAVINGS ANALYSIS
-----------------------------------------------------
- Human BPO Benchmark Rate: $24.00 / hour ($0.40 / min)
- AgentCall Autonomous AI Rate: $0.11 / min
- Modeled Monthly Calls: ${targetCallsPerMonth.toLocaleString()}
- Total Minutes Delivered: ${Math.round((targetCallsPerMonth * avgCallMinutes)).toLocaleString()} minutes
- Human BPO Equivalent Cost: $${humanCost.toLocaleString()}
- AgentCall AI Direct Cost: $${aiCost.toLocaleString()}
- Net Monthly Cash Savings: $${netSavings.toLocaleString()}
- Operational Cost Reduction: ${savingsPct}% Net Margin Improvement

3. CAMPAIGN CONVERSION FUNNEL
-----------------------------------------------------
${funnel.map(f => `- ${f.stage.toUpperCase()}: ${f.count.toLocaleString()} leads (${f.pct}%)`).join("\n")}

4. HOURLY CALL EFFICIENCY PEAK WINDOWS
-----------------------------------------------------
- Primary Peak Window: 10:00 AM – 12:30 PM (52% Pickup Rate)
- Secondary Peak Window: 04:00 PM – 06:30 PM (47% Pickup Rate)
- Optimal Outbound Reaching Slot: Tuesday & Thursday mornings

=====================================================
Report generated by AgentCall AI Autonomous Voice Platform
=====================================================`;
    ...
    link.href = url;
    link.download = `AgentCall_AI_Executive_Report_${period}_${Date.now()}.txt`;
    link.click();
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="flex-1 p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-3.5 py-1.5 rounded-xl text-xs font-bold bg-brand-500 hover:bg-brand-600 text-white shadow-md shadow-brand-500/20 transition-all flex items-center gap-1.5"
            >
        ...
        {/* Executive BPO vs AI ROI Savings Calculator */}
        <Card className="p-6 panel-card border-brand-500/25 bg-gradient-to-r from-brand-500/5 via-purple-500/5 to-emerald-500/5 shadow-lg">
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
            <div className="space-y-2 flex-1">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-xl bg-brand-500/20 flex items-center justify-center text-brand-500 font-bold">
                  <Calculator className="w-4 h-4" />
                </div>
                <div>
                  <h3 className="text-base font-bold text-slate-900 dark:text-white">
                    Executive BPO Replacement & ROI Calculator
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-white/50">
                    Calculates direct labor cost savings of Autonomous AI Voice Employees vs traditional human call centers ($24/hr BPO benchmark).
                  </p>
                </div>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4 pt-2">
                <div>
                  <div className="flex justify-between text-xs font-semibold mb-1">
                    <span className="text-slate-600 dark:text-white/70">Monthly Call Volume:</span>
                    <span className="text-brand-600 dark:text-brand-400 font-mono font-bold">{targetCallsPerMonth.toLocaleString()} calls</span>
                  </div>
                  <input
                    type="range"
                    min={500}
                ...
                <div>
                  <div className="flex justify-between text-xs font-semibold mb-1">
                    <span className="text-slate-600 dark:text-white/70">Average Duration per Call:</span>
                    <span className="text-brand-600 dark:text-brand-400 font-mono font-bold">{avgCallMinutes} minutes</span>
                  </div>
                  <input
                    type="range"
                    min={1}
            ...
            <div className="flex flex-wrap sm:flex-nowrap items-center gap-3 p-4 rounded-2xl bg-white dark:bg-black/40 border border-slate-200 dark:border-white/10 shadow-sm flex-shrink-0">
              <div className="text-center px-3 border-r border-slate-200 dark:border-white/10">
                <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400 dark:text-white/40 block">
                  Human BPO Cost
                </span>
              ...
              <div className="text-center px-3">
                ...
                <span className="text-xl font-black text-emerald-600 dark:text-emerald-400 font-mono flex items-center justify-center gap-0.5">
                  <ArrowUpRight className="w-4 h-4" /> ${netSavings.toLocaleString()}
                </span>
                <span className="text-[10px] font-bold text-emerald-500">
                  {savingsPct}% Margin Expansion
                </span>
              </div>
            </div>
          </div>
        ...
        {/* 24-Hour Peak Pickup & Reachability Heatmap */}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-slate-900 dark:text-white">
                  24-Hour Reachability & Peak Outreach Matrix
                </CardTitle>
              </div>
              <span className="text-xs text-emerald-600 dark:text-emerald-400 font-semibold flex items-center gap-1">
                <CheckCircle2 className="w-3.5 h-3.5" /> Optimal Call Slots: 10 AM - 12 PM & 4 PM - 6 PM
              </span>
            </div>
          </CardHeader>
          <div className="mt-4">
            <div className="grid grid-cols-12 sm:grid-cols-24 gap-1 h-16 items-end">
              {[
                { hour: "00", pct: 4 }, { hour: "01", pct: 2 }, { hour: "02", pct: 1 }, { hour: "03", pct: 1 },
                { hour: "04", pct: 2 }, { hour: "05", pct: 5 }, { hour: "06", pct: 9 }, { hour: "07", pct: 16 },
                { hour: "08", pct: 24 }, { hour: "09", pct: 38 }, { hour: "10", pct: 54 }, { hour: "11", pct: 51 },
                { hour: "12", pct: 44 }, { hour: "13", pct: 33 }, { hour: "14", pct: 29 }, { hour: "15", pct: 37 },
                { hour: "16", pct: 48 }, { hour: "17", pct: 49 }, { hour: "18", pct: 41 }, { hour: "19", pct: 34 },
                { hour: "20", pct: 27 }, { hour: "21", pct: 19 }, { hour: "22", pct: 11 }, { hour: "23", pct: 6 },
              ].map((h) => (
                <div key={h.hour} className="flex flex-col items-center gap-1 group relative">
                  <div
                    style={{ height: `${Math.max(6, Math.round(h.pct * 0.9))}px` }}
                  ...
                  />
                  <span className="text-[9px] font-mono text-slate-400 dark:text-white/40">{h.hour}</span>
                </div>
              ))}
            </div>
          </div>
