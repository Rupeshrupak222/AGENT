  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Active Seats", value: `${members.length} / 10`, color: "text-white" },
          { label: "Admins", value: members.filter(m => m.role === "company_admin").length, color: "text-brand-400" },
          { label: "Managers", value: members.filter(m => m.role === "manager").length, color: "text-purple-400" },
          { label: "Agents", value: members.filter(m => m.role === "agent").length, color: "text-emerald-400" },
        ].map((s) => (
      ...
      {/* Team table */}
      <div className="rounded-2xl p-6 bg-gradient-to-b from-white/[0.06] to-white/[0.02] border border-white/[0.08] shadow-2xl space-y-4">
          ...
          {members.map((member) => (
            ...
            >
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-xs text-white/30 hidden sm:block">Joined {member.joinedAt}</span>
                {member.role !== "company_admin" && (
                  <button
                    onClick={() => removeMember(member.id)}
                    className="p-1.5 rounded-lg hover:bg-white/10 text-white/30 hover:text-rose-400 transition-colors"
                  >
