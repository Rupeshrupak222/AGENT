    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <div className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <p className="text-2xl font-bold font-mono text-slate-900 dark:text-white">{vectorTelemetry.totalSources}</p>
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">512-dim embedding spaces</p>
        </div>
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <p className="text-base font-bold font-mono text-slate-900 dark:text-white truncate">text-embedding-3</p>
          <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-mono mt-1">Cosine similarity index</p>
        </div>
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">Ready for real-time RAG</p>
        </div>
      </div>
      ...
      {loading && items.length === 0 ? (
        <div className="space-y-3">
          {[0, 1, 2].map((i) => (
            <div key={i} className="h-20 rounded-xl bg-slate-100 dark:bg-slate-900/60 animate-pulse border border-slate-200 dark:border-slate-800" />
          ))}
        </div>
      ) : items.length === 0 ? (
        <div className="rounded-2xl p-12 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <p className="text-sm font-bold text-slate-900 dark:text-white">No knowledge documents found</p>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-sm mx-auto">
            {canManage
              ? "Ingest FAQs, documentation, or product manuals to equip your agents with instant recall."
              : "Knowledge documents will appear here once ingested."}
          </p>
          {canManage && (
            <Button
              variant="primary"
              ...
              className="mt-4"
            >
              <Plus className="w-3.5 h-3.5 mr-1.5" /> Ingest First Document
            </Button>
          )}
        </div>
      ) : (
        <div className="space-y-3">
          {items.map((item) => {
            const tm = TYPE_META[item.type] ?? TYPE_META.custom;
            const sm = STATUS_META[item.status] ?? STATUS_META.processing;
            const TypeIcon = tm.icon;
            const chunkEstimate = Math.max(1, Math.round((item.content?.length || 200) / 350));

            return (
              <div
                key={item.id}
                className="rounded-xl p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-brand-500/40 dark:hover:border-brand-500/40 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 transition-all shadow-sm"
              >
                <button
                ...
                >
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-bold text-slate-900 dark:text-white truncate group-hover:text-brand-600 transition-colors">
                        {item.name}
                      </p>
                      <span className="text-[10px] font-mono text-brand-700 dark:text-brand-300 bg-brand-500/10 px-1.5 py-0.5 rounded border border-brand-500/20 flex-shrink-0">
                ...
                <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
                  ...
                  {item.agent && (
                    <span className="inline-flex items-center gap-1 text-[10px] font-mono px-2 py-0.5 rounded-md bg-brand-500/10 text-brand-700 dark:text-brand-300 border border-brand-500/25" title={`Linked to ${item.agent.name}`}>
                      <Bot className="w-2.5 h-2.5" /> {item.agent.name}
                    </span>
                  )}
                  {item.tags && item.tags.length > 0 && (
                    <span className="hidden md:flex items-center gap-1 text-[10px] font-mono text-slate-500 dark:text-slate-400">
                      {item.tags.slice(0, 2).map((t) => (
                        <span key={t} className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">#{t}</span>
                      ))}
                    </span>
                  )}
                  ...
                  {canManage && (
                    <button
                      ...
                      className="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:text-brand-600 transition-colors"
                      aria-label="Edit"
                    >
                  ...
                  {canManage && (
                    <button
                      ...
                      className="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:text-rose-500 disabled:opacity-50 transition-colors"
                      aria-label="Delete"
                    >
      ...
      {/* Preview Drawer */}
      {preview && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 dark:bg-black/80 backdrop-blur-sm" onClick={() => setPreview(null)}>
          <div
            className="w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-start justify-between gap-3 mb-4">
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-white">{preview.name}</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                  {TYPE_META[preview.type]?.label ?? humanize(preview.type)} · Last updated {new Date(preview.updatedAt).toLocaleString()}
                </p>
              </div>
              <button onClick={() => setPreview(null)} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-700 dark:text-white/40 dark:hover:text-white transition-colors" aria-label="Close preview">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              {preview.sourceUrl && (
                <a
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-brand-500/10 text-brand-700 dark:text-brand-400 border border-brand-500/20 hover:bg-brand-500/20 transition-all"
                >
            ...
            <div className="rounded-xl p-4 bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 whitespace-pre-wrap text-xs font-mono leading-relaxed text-slate-800 dark:text-slate-200 max-h-96 overflow-y-auto">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 dark:bg-black/80 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setShowForm(false)} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-700 dark:text-white/40 dark:hover:text-white transition-colors" aria-label="Close">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div>
                <label htmlFor="kb-name" className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Source Title *</label>
                <input
                  id="kb-name"
                  value={form.name}
              ...
              <div>
                <label htmlFor="kb-type-sel" className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Document Type</label>
                <select
                  id="kb-type-sel"
                  value={form.type}
              ...
              <div>
                <label htmlFor="kb-url" className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Source URL (Optional if pasting content)</label>
                <input
                  id="kb-url"
              ...
              <div>
                <label htmlFor="kb-content" className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Training Text / FAQs</label>
                <textarea
                  id="kb-content"
                  value={form.content}
                  ...
                  className="w-full rounded-lg px-3 py-2.5 text-xs font-mono bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white placeholder-slate-400 outline-none focus:border-brand-500 resize-y shadow-xs"
                />
              </div>

              <div>
                <label htmlFor="kb-tags" className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Tags (Comma-separated)</label>
                <input
                  id="kb-tags"
                  value={form.tags}
              ...
              <div>
                <label htmlFor="kb-agent" className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Link to Specific Agent</label>
                <select
                  id="kb-agent"
                  value={form.agentId}
