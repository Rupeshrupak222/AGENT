...
export function LiveAirTrafficControl({ recentCalls, agents = [] }: LiveAirTrafficControlProps) {
  ...
  return (
    <div className="rounded-2xl p-5 bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 border border-slate-800 text-white shadow-xl relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-white/10 relative z-10">
        ...
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl text-xs font-mono font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/25">
            ...
            >
              ...
              <div>
                <div className="flex items-start justify-between gap-2">
                  ...
                  >
                    {isEscalating && <AlertCircle className="w-3 h-3 text-rose-400" />}
                    {call.sentiment}
                  </span>
                </div>
              ...
              <div className="p-2.5 rounded-lg bg-black/40 border border-white/5 space-y-1.5">
                <div className="flex items-center justify-between text-[10px] font-mono text-slate-400">
                  <span className="flex items-center gap-1 text-slate-300 truncate">
                    <Activity className="w-3 h-3 text-brand-400" /> {call.topic}
                  </span>
        ...
        {isListenModalOpen && selectedCall && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <motion.div
              ...
              className="w-full max-w-2xl rounded-2xl bg-slate-900 border border-slate-700 shadow-2xl p-6 text-white relative space-y-5"
            >
              {/* Close Button */}
              <button
              ...
              <div className="p-4 rounded-xl bg-black/60 border border-white/10 space-y-3">
                <div className="flex items-center justify-between text-xs font-mono">
                  ...
                  <span className="text-slate-400">Opus 48kHz · Jitter 4ms</span>
                </div>
              ...
              <div className="p-4 rounded-xl bg-slate-950/70 border border-white/5 space-y-2 text-xs">
                <div className="flex items-center justify-between text-slate-400 font-mono text-[11px]">
                  ...
                  <span className="text-brand-400 font-bold">Latency: 280ms</span>
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 h-9 px-3 rounded-xl bg-black/40 border border-white/15 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-brand-500"
                    onKeyDown={(e) => {
                ...
                <div className="flex items-center gap-1.5 flex-wrap pt-1">
                  {[
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg text-[10px] font-medium bg-white/[0.06] hover:bg-white/[0.12] text-slate-300 hover:text-white border border-white/10 transition-colors"
                    >
                      + {chip}
                    </button>
                  ))}
                ...
                {whisperSuccess && (
                  <motion.div
                    ...
                    className="p-2 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs flex items-center gap-1.5"
                  >
              ...
              <div className="flex items-center justify-between gap-3 pt-3 border-t border-white/10">
                <button
                  onClick={() => setIsListenModalOpen(false)}
                  className="px-4 h-9 rounded-xl text-xs font-semibold bg-white/[0.06] hover:bg-white/10 text-slate-300 transition-colors"
                >
                  Close Monitor
                </button>
