    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold px-3 py-1.5 rounded-xl bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Bookings This Week", value: "24 Demos", color: "text-emerald-600 dark:text-emerald-400" },
          { label: "Scheduled Tomorrow", value: "6 Calls", color: "text-brand-600 dark:text-brand-400" },
          { label: "Show-Up Ratio", value: "88.5%", color: "text-cyan-600 dark:text-cyan-400" },
          { label: "Avg Booking Call Time", value: "2m 14s", color: "text-purple-600 dark:text-purple-400" },
        ].map((s) => (
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center bg-slate-100 dark:bg-white/[0.04] border border-slate-200 dark:border-white/10 rounded-xl p-1">
        ...
        <div className="space-y-3">
          {filtered.map((item) => (
            <div
              key={item.id}
              className="rounded-2xl p-5 bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.08] hover:border-brand-500/30 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm"
            >
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-white/50 mt-1 flex items-center gap-2 flex-wrap">
                    ...
                    <span>·</span>
                    <span>Booked by: <strong className="text-brand-600 dark:text-brand-300">{item.agentName}</strong></span>
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 self-end sm:self-auto flex-shrink-0 pt-3 sm:pt-0 border-t sm:border-t-0 border-slate-200 dark:border-white/[0.05] w-full sm:w-auto justify-between sm:justify-end">
                <div className="text-right">
                  <p className="text-xs font-mono font-bold text-slate-900 dark:text-white flex items-center gap-1">
                    <Clock className="w-3.5 h-3.5 text-brand-500 dark:text-brand-400" /> {item.date} at {item.time}
                  </p>
                  <p className="text-[10px] text-slate-500 dark:text-white/40 mt-0.5">{item.duration} video demo</p>
                </div>
                <a
                  href={`tel:${item.phone}`}
                  className="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-white/[0.06] hover:bg-slate-50 dark:hover:bg-white/[0.12] text-xs font-semibold text-slate-700 dark:text-white border border-slate-200 dark:border-white/10 flex items-center gap-1.5 transition-colors"
                >
