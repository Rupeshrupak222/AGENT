...
const TRIGGER_LABELS: Record<string, { label: string; color: string }> = {
  call_completed: { label: "Call Completed", color: "bg-blue-500/15 text-blue-400 border-blue-500/30" },
  lead_qualified: { label: "Lead Qualified (Score > 75)", color: "bg-purple-500/15 text-purple-400 border-purple-500/30" },
  call_missed:    { label: "Call Missed / No Answer", color: "bg-amber-500/15 text-amber-400 border-amber-500/30" },
  deal_closed:    { label: "Appointment Booked", color: "bg-emerald-500/15 text-emerald-400 border-emerald-500/30" },
};

const ACTION_ICONS: Record<string, { icon: any; label: string; color: string }> = {
  whatsapp:   { icon: MessageSquare, label: "WhatsApp Message", color: "text-emerald-400 bg-emerald-500/10" },
  sms:        { icon: PhoneCall,     label: "SMS Text",        color: "text-cyan-400 bg-cyan-500/10" },
  email:      { icon: Mail,          label: "Email Notice",    color: "text-amber-400 bg-amber-500/10" },
  webhook:    { icon: Webhook,       label: "Webhook Dispatch",color: "text-brand-400 bg-brand-500/10" },
  crm_update: { icon: RefreshCw,     label: "CRM Property",    color: "text-purple-400 bg-purple-500/10" },
};
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Active Workflows", value: rules.filter(r => r.status === "active").length, color: "text-emerald-400" },
          { label: "Total Triggers Sent", value: "4,240", color: "text-brand-400" },
          { label: "Delivery Success Rate", value: "99.4%", color: "text-cyan-400" },
          { label: "Average Latency", value: "1.2s", color: "text-purple-400" },
        ].map((s) => (
        ...
        <div className="space-y-3">
          {rules.map((rule) => {
            ...
            return (
              <div
                key={rule.id}
                className="rounded-2xl p-5 bg-gradient-to-b from-white/[0.05] to-white/[0.02] border border-white/[0.08] hover:border-brand-500/30 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xl"
              >
                ...
                <div className="flex items-center gap-4 self-end md:self-auto flex-shrink-0 pt-3 md:pt-0 border-t md:border-t-0 border-white/[0.05] w-full md:w-auto justify-between md:justify-end">
                  <div className="text-right">
                    ...
                    <p className="text-[10px] text-white/40">{rule.lastRun}</p>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-xl hover:bg-white/[0.06] text-white/30 hover:text-rose-400 transition-colors"
                    >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full h-10 rounded-xl px-3 text-xs bg-[#1a0406] border border-white/15 text-white outline-none"
                    >
